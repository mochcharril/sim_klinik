...
@section('content')
<div>
    <a href="{{url('/pharmacist/master-data/medicine_rule/add')}}" class="text-white rounded px-4 py-3 mt-2" style="background-color: #609966;">Tambah Data</a>
        ...
        <table id="thisTable" class="stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
            <thead>
                <tr>
                    ...
                    <th data-priority="2">Nama Obat</th>
                    <th data-priority="3">Aksi</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($getMedicineRule as $item)  
                <tr>
                    <td class="text-center">{{$loop->iteration}}</td>
                    <td class="text-left">{{$item->name}}</td>
                    <td class="text-left t-mx-3 flex">
                        <form action="{{url('/pharmacist/master-data/medicine_rule')}}/{{$item->id}}/edit" method="POST" class="m-auto">
                            @csrf
                        ...
                        <form action="{{url('/pharmacist/master-data/medicine_rule')}}/{{$item->id}}/drop" method="POST" class="m-auto">
                            @method('delete')
                            @csrf
