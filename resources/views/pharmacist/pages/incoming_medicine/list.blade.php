...
@section('content')
<div>
    <a href="{{url('/pharmacist/warehouse/incoming_medicine/add')}}" class="text-white rounded px-4 py-3 mt-2" style="background-color: #609966;">Tambah Data</a>
        ...
        <table id="thisTable" class="stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
            <thead>
                <tr>
                    ...
                    <th data-priority="2">Kode Transaksi Obat Masuk</th>
                    <th data-priority="3">Apoteker Bertugas</th>
                    <th data-priority="4">Total</th>
                    <th data-priority="5">Tanggal Transaksi</th>
                    <th data-priority="6">Aksi</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($getIncomingMedicine as $item)  
                <tr>
                    <td class="text-center">{{$loop->iteration}}</td>
                    <td class="text-left">{{$item->code_im}}</td>
                    <td class="text-left">{{$item->pharmacist_name}}</td>
                    <td class="text-left">{{$item->total}}</td>
                    <td class="text-left">{{$item->date_income_medicine}}</td>
                    <td class="text-left t-mx-3 flex">
                        <form action="{{url('/pharmacist/warehouse/incoming_medicine')}}/{{$item->id}}/detail" method="POST" class="m-auto">
                            @csrf
