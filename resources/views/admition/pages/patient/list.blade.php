...
@section('content')
<div>
    <a href="{{url('/admition/master-data/patient/add')}}" class="text-white rounded px-4 py-3 mt-2" style="background-color: #609966;">Tambah Data</a>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-left t-mx-3 flex">
                        <form action="{{url('/admition/master-data/patient')}}/{{$item->id}}/edit" method="POST" class="m-auto">
                            @csrf
                        ...
                        <form action="{{url('/admition/master-data/patient')}}/{{$item->id}}/drop" method="POST" class="m-auto">
                            @method('delete')
                            @csrf
                            <button class="bg-red-600 w-6 p-5 text-sm font-bold tracking-wider text-white rounded-full hover:bg-red-700 inline-flex items-center justify-center"  onclick="return confirm('Hapus Data ?')">
                                <i class="fa fa-trash"></i>
                            </button>
                        </form>
                        <span class="m-auto">|</span>
                        <form action="{{url('/admition/master-data/patient')}}/{{$item->id}}/print" method="POST" class="m-auto">
                            @csrf
    ...
    <div class="p-8 mt-6 lg:mt-0 rounded shadow bg-white bg-opacity-90">
        ...
        <table id="thisTables" class="stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
            ...
            <tbody>
                @foreach ($getPatientRetention as $item)  
                <tr>
                    <td class="text-center">{{$loop->iteration}}</td>
                    <td class="text-left">{{$item->code_rm}}</td>
                    <td class="text-left">{{$item->name}}</td>
